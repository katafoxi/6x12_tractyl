...
bool led_update_user(led_t led_state) {
    // Управление светодиодом CapsLock
    if (led_state.caps_lock) {
        writePinHigh(LED_CAPS_LOCK_PIN);  // Включить светодиод
    } else {
        writePinLow(LED_CAPS_LOCK_PIN);   // Выключить светодиод
    }
    return true;  // Разрешить стандартную обработку
}
